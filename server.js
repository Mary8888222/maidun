...
function seed() {
  ...
  try {
    ...
    const plots = [
      ['MD-001','成都崇州核心试验田','四川省成都市崇州市',1260,'川麦104',103.673,30.631,'高','重点监测','唐琦翔'],
      ['MD-002','德阳广汉智能农场','四川省德阳市广汉市',980,'川麦93',104.282,30.977,'中','监测中','陈婉'],
      ['MD-003','绵阳江油PCR联合示范田','四川省绵阳市江油市',860,'绵麦902',104.745,31.778,'低','生长正常','何佳萍'],
      ['MD-004','南充营山机器人巡检基地','四川省南充市营山县',1420,'川育27',106.690,30.931,'中','任务执行中','蒙璐'],
      ['MD-005','重庆潼南绿色防控示范田','重庆市潼南区',760,'川麦55',105.840,30.190,'低','防控完成','彭慧琼'],
      ['MD-006','重庆梁平孢子捕获观测站','重庆市梁平区',1150,'蜀麦1868',107.800,30.670,'高','预警处置中','唐婉婷']
    ];
    ...
    [
      ['CAM-01',1,'崇州机器人移动视角','在线','1080P','2026-06-22 12:30'],
      ['CAM-02',2,'广汉麦田全景监控','在线','4K','2026-06-22 12:30'],
      ['CAM-03',4,'营山固定监控','在线','1080P','2026-06-22 12:29'],
      ['CAM-04',6,'梁平孢子站监控','维护','1080P','2026-06-22 09:16']
...
function syncRegionalDemoData() {
  const regionalPlots = [
    ['成都崇州核心试验田','四川省成都市崇州市',103.673,30.631,'MD-001'],
    ['德阳广汉智能农场','四川省德阳市广汉市',104.282,30.977,'MD-002'],
    ['绵阳江油PCR联合示范田','四川省绵阳市江油市',104.745,31.778,'MD-003'],
    ['南充营山机器人巡检基地','四川省南充市营山县',106.690,30.931,'MD-004'],
    ['重庆潼南绿色防控示范田','重庆市潼南区',105.840,30.190,'MD-005'],
    ['重庆梁平孢子捕获观测站','重庆市梁平区',107.800,30.670,'MD-006']
  ];
  const updatePlot = db.prepare('UPDATE plots SET name=?,region=?,longitude=?,latitude=? WHERE code=?');
  regionalPlots.forEach(plot => updatePlot.run(...plot));
  const regionalChannels = [
    ['崇州机器人移动视角',1,'CAM-01'],['广汉麦田全景监控',2,'CAM-02'],
    ['营山固定监控',4,'CAM-03'],['梁平孢子站监控',6,'CAM-04']
  ];
  const updateChannel = db.prepare('UPDATE video_channels SET name=?,plot_id=? WHERE channel_code=?');
  regionalChannels.forEach(channel => updateChannel.run(...channel));
}
syncRegionalDemoData();
